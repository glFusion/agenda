...
require_once $_CONF['path'].'/plugins/agenda/autoinstall.php';

USES_lib_install();

if (!SEC_inGroup('Root')) {
    COM_errorLog("Someone has tried to access the Agenda install/uninstall page.  User id: {$_USER['uid']}, Username: {$_USER['username']}, IP: $REMOTE_ADDR",1);
...
if (SEC_checkToken()) {
    ...
    if ($action == 'install') {
    	...
    	if (plugin_uninstall_agenda('installed')) {
    		/**
    		* Redirects to the plugin editor
    		*/
